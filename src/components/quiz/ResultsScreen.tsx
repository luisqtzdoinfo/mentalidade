...
const resultLevels = {
  MIND_OF_STEEL: {
    ...
    icon: <ShieldCheck className="h-12 w-12 md:h-16 md:w-16 text-accent" />,
  },
  MIND_IN_CONSTRUCTION: {
    title: "Mente em Construção",
    icon: <Construction className="h-12 w-12 md:h-16 md:w-16 text-accent" />,
  },
  FRAGMENTED_MIND: {
    title: "Mente Fragmentada",
    icon: <BrainCircuit className="h-12 w-12 md:h-16 md:w-16 text-accent" />,
  },
};
...
export function ResultsScreen({ results, onNext }: ResultsScreenProps) {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto text-center shadow-2xl border-2 border-primary/10 animate-in fade-in duration-1000">
      <CardHeader className="items-center p-4 md:p-10">
        {icon}
        <CardTitle className="text-3xl md:text-4xl font-black mt-4">{title}</CardTitle>
      </CardHeader>
      ...
      <CardFooter className="flex justify-center p-4 md:p-10">
        <Button size="lg" onClick={onNext} className="bg-accent text-accent-foreground hover:bg-accent/90 rounded-full text-base md:text-lg font-bold px-8 md:px-10 py-5 md:py-6 group text-center">
          {results.cta}
