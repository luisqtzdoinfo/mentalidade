  ...
  return (
    <Card key={question.id} className="w-full max-w-3xl mx-auto shadow-2xl border-2 border-primary/10 animate-in fade-in-50 slide-in-from-bottom-10 duration-500">
      ...
      <CardContent className="p-4 md:p-10">
        <h2 className="text-xl md:text-3xl font-bold text-center mb-6 md:mb-8 min-h-[5rem] md:min-h-[6rem] flex items-center justify-center">
          {question.question}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          {question.answers.map((answer, index) => (
            <Button
              ...
              size="lg"
              className={cn(
                "justify-start text-left h-auto py-3 px-4 text-sm md:text-base whitespace-normal border-2 hover:border-accent hover:bg-accent/10 focus:bg-accent/20 focus:border-accent transition-all duration-300",
